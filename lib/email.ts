 ...
 */
function extractImagesFromHtml(html: string): Array<{ src: string; alt?: string; isBase64: boolean }> {
...
function convertHtmlForEmail(
  ...
  cleanHtml = cleanHtml
    // Parágrafos
    .replace(/<p[^>]*>/gi, '<p style="margin: 0 0 12px 0; line-height: 1.6; color: #111827;">')
    // Negrito
    ...
    // Links
    .replace(/<a[^>]+href=["']([^"']+)["'][^>]*>/gi, '<a href="$1" style="color: #7C3AED; text-decoration: underline;">')
    // Listas
    .replace(/<ul[^>]*>/gi, '<ul style="margin: 12px 0; padding-left: 24px; list-style-type: disc;">')
    .replace(/<ol[^>]*>/gi, '<ol style="margin: 12px 0; padding-left: 24px; list-style-type: decimal;">')
    .replace(/<li[^>]*>/gi, '<li style="margin: 4px 0; line-height: 1.6;">')
    // Títulos
    .replace(/<h1[^>]*>/gi, '<h1 style="font-size: 20px; font-weight: 700; margin: 16px 0 12px 0; color: #111827;">')
    .replace(/<h2[^>]*>/gi, '<h2 style="font-size: 18px; font-weight: 700; margin: 14px 0 10px 0; color: #111827;">')
    .replace(/<h3[^>]*>/gi, '<h3 style="font-size: 16px; font-weight: 600; margin: 12px 0 8px 0; color: #111827;">')
    // Quebras de linha
    .replace(/<br[^>]*>/gi, '<br>')
...
export async function sendNewTicketEmail({
  ...
  try {
    ...
    const allImageAttachments: Array<{ filename: string; url?: string; content?: Buffer; cid?: string; contentType?: string; isBase64: boolean }> = []
    ...
    const appBaseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://ativadesk.grupoativa.net:19831"
