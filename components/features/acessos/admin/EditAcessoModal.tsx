...
type EditAcessoFormData = z.infer<typeof editAcessoSchema>
...
interface EditAcessoModalProps {
...
export function EditAcessoModal({
  acesso,
  open,
  onOpenChange,
  onSuccess,
}: EditAcessoModalProps) {
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<EditAcessoFormData>({
    resolver: zodResolver(editAcessoSchema),
    defaultValues: {
      nome: acesso.nome,
      email: acesso.email || "",
      senha: acesso.senha || "",
      departamento: acesso.departamento || "",
      categoriaId: acesso.categoriaId || "none",
    },
  })
  ...
  const onSubmit = async (data: EditAcessoFormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(response.error || "Erro ao atualizar acesso")
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-[600px] max-h-[90vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader className="space-y-1.5 sm:space-y-2">
          <DialogTitle className="text-lg sm:text-xl">Editar Acesso</DialogTitle>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 sm:space-y-4">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground leading-relaxed">
              Senha definida para o acesso a sistemas ou redes.
            </p>
          </div>
