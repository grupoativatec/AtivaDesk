...
export function CategoriasDialog({ open, onOpenChange }: CategoriasDialogProps) {
  ...
  const fetchCategorias = async () => {
    setIsLoading(true)
    try {
      const res = await fetch("/api/admin/colaboradores/categorias")
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/colaboradores/categorias/${isDeleting}`, {
        method: "DELETE",
      })
        ...
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-[600px] max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          <DialogHeader className="space-y-1.5 sm:space-y-2">
            ...
            <DialogDescription className="text-xs sm:text-sm">
              Crie e gerencie categorias para colaboradores externos (ex: Itajaí, São Francisco, etc.)
            </DialogDescription>
          </DialogHeader>
